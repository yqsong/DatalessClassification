...
//		ConceptTreeTopDownML tree = new ConceptTreeTopDownML(data, method, conceptWeights, false);
		...
		for (String docID : corpusContentProc.getCorpusConceptVectorMap().keySet()) {
			...
			SparseVector vec = corpusContentProc.getCorpusConceptVectorMap().get(docID);
			for (String label : tree.getTreeNodeVectorHash().keySet()) {
				SparseVector labelVec = tree.getTreeNodeVectorHash().get(label);
				double cos = vec.cosine(labelVec, conceptWeights);
				System.out.print(label + "," + String.format("%.4f", cos) + "; ");
			}
			System.out.println();
		}
