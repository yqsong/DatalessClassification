...
public class SemanticGraphCut {
	...
	public static HashMap<Integer, Double> combineVectorsGraph (List<HashMap<Integer, Double>> vectorList, 
		...
		for (int i = 0; i < clusters.size(); ++i) {
			...
			if (clusterIds.size() > 0) {
				clusterVector = add(clusterVector, vectorList.get(clusterIds.get(0)), 1, scoreList.get(clusterIds.get(0)));
				
				if (clusterIds.size() > 1) {
					for (int j = 1; j < clusterIds.size(); ++j) {
						clusterVector = addWithBothNonZero(clusterVector, vectorList.get(clusterIds.get(j)), 1, scoreList.get(clusterIds.get(j)));
					}
					
					double decayFactor = (double) 1 / Math.log(clusterIds.size() + Math.exp(1));
					for (int j = 0; j < clusterIds.size(); ++j) {
						clusterVector = addWithDecayFactor(clusterVector, vectorList.get(clusterIds.get(j)), scoreList.get(clusterIds.get(j)) * decayFactor);
					}
				}
	...
	public static HashMap<Integer, Double> combineVectorsSimple (List<HashMap<Integer, Double>> vectorList, List<Double> scoreList) {
...
//        		edgeWeight[i][j] = cosine(vectorList.get(i), vectorList.get(j), vectorNormList.get(i), vectorNormList.get(j));
	...
	private static HashMap<Integer, Double> add (HashMap<Integer, Double> v1, HashMap<Integer, Double> v2, double weight1, double weight2) {
	...
	private static HashMap<Integer, Double> addWithDecayFactor (HashMap<Integer, Double> sumVector, HashMap<Integer, Double> additionalVector, double decayFactor) {
		HashMap<Integer, Double> v3 = new HashMap<Integer, Double>();
	...
	private static HashMap<Integer, Double> addWithBothNonZero (HashMap<Integer, Double> v1, HashMap<Integer, Double> v2, double weight1, double weight2) {
		HashMap<Integer, Double> v3 = new HashMap<Integer, Double>();
		if (v1.size() < v2.size()) {
			for (Integer key : v1.keySet()) {
				if (v2.containsKey(key) == true) {
