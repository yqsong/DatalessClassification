...
public class MemoryBasedESA extends AbstractESA {
	...
	public MemoryBasedESA () {
		...
		System.out.println("[Read Memory ESA Data] " + DatalessResourcesConfig.memorybasedESA);
		
		File inputFile = null;
		
		inputFile = new File(DatalessResourcesConfig.memorybasedESA);
	...
	public Hashtable<Integer, Double> getConceptVectorBasedonTFIDF(HashMap<String, Double> queryTFIDF, int topK, String vectorField){
    	...
    	try {
	        ...
	        for(String strTerm : queryTFIDF.keySet()) { 
	    		...
	    		if (termConceptMap.size() > 0 && queryTFIDF.containsKey(strTerm) == true && queryTFIDF.get(strTerm) > 0) {
	    			conceptMapList.add(termConceptMap);
	   ...
	   public double cosin(String s1,String s2){
		...
		@Override
		public List<ConceptData> retrieveConcepts(String document,
				int numConcepts) throws Exception {
